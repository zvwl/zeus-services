...
export function LoginForm({
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      // replay a redirect the client router cached before the auth state
      // changed (see verify-2fa/page.tsx). A document request re-runs
      // middleware with the fresh session cookies.
      window.location.assign(next);
    } catch {
      // Chunk-load failure (deploy skew, dropped connection) — without this
      // the form spins on "Signing in…" forever.
