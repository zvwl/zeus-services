...
import { Amplify } from "npm:aws-amplify@6";
import { signIn, signOut, fetchAuthSession } from "npm:aws-amplify/auth";
    ...
    Cognito: {
      ...
      loginWith: {
        oauth: {
          ...
          redirectSignOut: ["https://eldorado.gg"],
          responseType: "code",
        },
      },
    },
...
async function cognitoSRPAuth(email: string, password: string): Promise<{
  ...
  if (!idToken) throw new Error("Amplify login succeeded but no ID token was returned");

  return { idToken, refreshToken: "", expiresIn: 3600 };
}

// ── Cognito refresh (direct API — no Amplify needed) ─────────────────────────
...
async function callEldorado(idToken: string, method: string, endpoint: string, body?: unknown) {
  ...
  console.log("ELDORADO", method, endpoint, "→", res.status, JSON.stringify(data)?.slice(0, 300));
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "call_api") {
      ...
      // Always return HTTP 200 — frontend checks result.ok/result.status
