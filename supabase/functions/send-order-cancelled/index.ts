...
<div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); padding: 32px 20px; text-align: center; color: white;">
    ...
    <p style="margin: 8px 0 0 0; font-size: 14px; opacity: 0.9;">Order Cancelled</p>
  </div>
  ...
  <div style="padding: 32px 20px; max-width: 640px; margin: 0 auto; background: #f8fafc;">
    ...
    <p style="font-size: 15px; color: #555; margin: 0 0 16px;">Your order has been cancelled. If you have any questions or concerns, our team is here to help.</p>
    ...
    <div style="background: white; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px 18px; margin: 20px 0;">
      ...
      <p style="margin: 4px 0; font-size: 14px;"><strong>Amount:</strong> ${formatCurrency(total_amount, currency)}</p>
    </div>
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: customer_email,
        subject: `Order Cancelled #${String(orderId).slice(0, 8)}`,
        html
      })
    });
    ...
    return new Response(JSON.stringify({ error: "Failed to send order cancelled email" }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" }
    });
  }
